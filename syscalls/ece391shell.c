...
void do_tab(uint8_t *cmd)
{
    ...
    {
        if (last_pos != -1)
            ece391_ioctl(1, IOCTL_SIMKB, firstCmd);
        
        ece391_ioctl(1, IOCTL_SIMKB, matches[0]);
    ...
    {
        noMatches = 1;
        ece391_ioctl(1, IOCTL_SIMKB, cmd);
    }
}
...
int main()
{
    ...
    next:
        ...
        if (restore_last == 1)
        {
            ece391_ioctl(1, IOCTL_SIMKB, buf);
            restore_last = 0;
        }
        ...
        if (ece391_strcmp((const uint8_t *)buf, (const uint8_t *)"^[[A") == 0)
        {
            ...
            if (cmdQueue.head == 0 && cmdQueue.tail == 0)
                goto next;
            ece391_ioctl(1, IOCTL_SIMKB, cmdQueue.cmd[cmdQueue.curIdx]);
            goto next;
        }
